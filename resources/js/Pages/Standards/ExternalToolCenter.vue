...
<template>
  <EraLmsLayout :session-user="sessionUser" @logout="$emit('logout')">
    <template #breadcrumb>Learning Standards / External Tool Center</template>
    <section class="mx-auto max-w-7xl px-6 py-5">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-lg font-semibold">External Tool Center</h1>
          <p class="mt-1 text-sm text-slate-600">Registry cho Zoom, Teams, Google Meet, Lab, AI Tool và publisher content.</p>
        </div>
        <button class="rounded-md bg-slate-950 px-3 py-2 text-sm text-white">Add Tool</button>
      </div>
      ...
      <div class="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="tool in tools" :key="tool.id" class="border bg-white p-4">
          <div class="flex items-start justify-between gap-3"><div><h2 class="text-sm font-semibold">{{ tool.name }}</h2><p class="mt-1 text-sm text-slate-600">{{ tool.provider }} · {{ tool.category }}</p></div><span class="rounded bg-slate-100 px-2 py-1 text-xs">{{ tool.status }}</span></div>
