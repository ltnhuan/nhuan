...
async function load() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.message || 'Không tải được sync jobs.')
...
<template>
  <EraLmsLayout :session-user="sessionUser">
    <template #breadcrumb>Integration Hub / Sync Jobs</template>
    <section class="mx-auto max-w-7xl px-4 py-5 sm:px-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-lg font-semibold">Sync Jobs</h1>
      ...
      <div class="mt-5 overflow-hidden border bg-white">
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th class="px-4 py-3">Job type</th><th class="px-4 py-3">Entity</th><th class="px-4 py-3">Status</th><th class="px-4 py-3">Total</th><th class="px-4 py-3">Processed</th><th class="px-4 py-3">Started</th></tr></thead>
          <tbody class="divide-y">
            <tr v-if="loading"><td class="px-4 py-6 text-slate-500" colspan="6">Đang tải dữ liệu...</td></tr>
            <tr v-else-if="!jobs.length"><td class="px-4 py-6 text-slate-500" colspan="6">Chưa có sync job.</td></tr>
