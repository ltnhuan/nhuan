...
const cards = computed(() => [
  ['Integration systems', total(health.value.systems), statusText(health.value.systems)],
  ['Events', total(health.value.events), statusText(health.value.events)],
  ['Webhook deliveries', total(health.value.deliveries), statusText(health.value.deliveries)],
  ['Open conflicts', health.value.conflicts_open || 0, 'mapping'],
])
...
function statusText(bucket) {
  ...
  return keys.length ? keys.join(', ') : 'no data'
}
...
async function load() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.message || 'Không tải được health integration.')
    health.value = data
  } catch (exception) {
...
<template>
  <EraLmsLayout :session-user="sessionUser">
    <template #breadcrumb>Integration Hub / Dashboard</template>
    <section class="mx-auto max-w-7xl px-4 py-5 sm:px-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 class="text-lg font-semibold">Integration Dashboard</h1>
          <p class="mt-1 text-sm text-slate-600">Theo dõi SIS connectivity, event backlog, queue health và kết quả sync.</p>
        </div>
        <button class="rounded-md bg-slate-950 px-3 py-2 text-sm text-white" :disabled="loading" @click="load">Health check</button>
      </div>
      ...
      <div class="mt-5 grid gap-4 lg:grid-cols-[1fr_360px]">
        <div class="border bg-white p-4">
          <h2 class="text-sm font-semibold">Event status distribution</h2>
          <div v-if="!loading && !bars.length" class="mt-4 rounded-md bg-slate-50 p-4 text-sm text-slate-500">Chưa có integration event.</div>
          <div v-else class="mt-4 space-y-3">
            <div v-for="bar in bars" :key="bar[0]">
              <div class="mb-1 flex justify-between text-sm"><span>{{ bar[0] }}</span><span>{{ bar[2] }} bản ghi</span></div>
