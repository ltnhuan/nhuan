...
<template>
  <EraLmsLayout :session-user="sessionUser">
    <template #breadcrumb>Điểm danh online / Live sessions</template>
    <section class="border-b bg-white">
      <div class="mx-auto max-w-7xl px-4 py-5 sm:px-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div><h1 class="text-lg font-semibold">Live Session Manager</h1><p class="mt-1 text-sm text-slate-600">Lịch livestream, workshop và connector theo cấu hình trong dữ liệu hệ thống.</p></div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-5 sm:px-6">
      ...
      <div class="overflow-hidden border bg-white">
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th class="px-4 py-3">Buổi học</th><th class="px-4 py-3">Course</th><th class="px-4 py-3">Provider</th><th class="px-4 py-3">Bắt đầu</th><th class="px-4 py-3">Kết thúc</th><th class="px-4 py-3">Trạng thái</th></tr></thead>
          <tbody class="divide-y">
            <tr v-if="loading"><td class="px-4 py-6 text-slate-500" colspan="6">Đang tải dữ liệu...</td></tr>
            <tr v-else-if="!sessions.length"><td class="px-4 py-6 text-slate-500" colspan="6">Chưa có live session.</td></tr>
