...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(data.message || 'Không gọi được API ma trận đề.')
...
async function load() {
  ...
  try {
    ...
    message.value = 'Đã tải ma trận đề.'
  } catch (error) {
...
async function createBlueprint() {
  ...
  try {
    ...
    message.value = 'Đã tạo ma trận đề.'
  } catch (error) {
    message.value = error.message
  }
}
...
async function generatePreview() {
  if (!selected.value) {
    message.value = 'Chọn ma trận đề trước khi sinh bản xem trước.'
    return
  }
  ...
  try {
    ...
    message.value = `Đã sinh bản xem trước ${count} câu.`
  } catch (error) {
    message.value = error.message
  }
}
  ...
  <EraLmsLayout>
    ...
    <section class="mx-auto grid max-w-7xl gap-4 px-6 py-5 lg:grid-cols-[1fr_380px]">
      <main class="min-w-0 space-y-4">
        <div class="border bg-white p-4">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 class="text-lg font-semibold">Ma trận đề thi</h1>
              <p class="mt-1 text-sm text-slate-600">Tạo ma trận đề và sinh bản xem trước bằng bộ sinh đề ngẫu nhiên.</p>
            </div>
        ...
        <div class="overflow-hidden border bg-white">
          ...
          <table v-else class="w-full text-left text-sm">
            <thead class="bg-slate-50 text-xs uppercase text-slate-500"><tr><th class="px-4 py-3">Mã</th><th class="px-4 py-3">Ma trận đề</th><th class="px-4 py-3">Số câu</th><th class="px-4 py-3">Thời lượng</th><th class="px-4 py-3">Trạng thái</th></tr></thead>
            <tbody class="divide-y">
              <tr v-for="item in blueprints" :key="item.id" class="cursor-pointer hover:bg-slate-50" :class="selected?.id === item.id ? 'bg-cyan-50' : ''" @click="selectedId = item.id">
                ...
                <td class="px-4 py-3">{{ statusLabel[item.status] || item.status }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="border bg-white p-4">
          <h2 class="text-sm font-semibold">Các phần của ma trận đề đang chọn</h2>
          <div class="mt-3 grid gap-3">
            <div v-for="section in sections" :key="section.name" class="grid gap-3 rounded-md border p-3 text-sm md:grid-cols-5">
              ...
              <div>{{ (section.difficulty || []).map((item) => difficultyLabel[item] || item).join(', ') }}</div>
              <div>{{ (section.bloom_level || []).map((item) => bloomLabel[item] || item).join(', ') }}</div>
              <div>{{ section.score_each }} điểm/câu</div>
            </div>
            <div v-if="!sections.length" class="rounded-md bg-slate-50 p-3 text-sm text-slate-500">Ma trận đề chưa có phần nào.</div>
          </div>
        </div>
      </main>
      ...
      <aside class="space-y-4">
        <div class="border bg-white p-4">
          <h2 class="text-sm font-semibold">Tạo ma trận đề nhanh</h2>
          <div class="mt-3 grid gap-3">
            <input v-model="form.code" class="rounded-md border px-3 py-2 text-sm" placeholder="Mã ma trận đề" />
            <input v-model="form.name" class="rounded-md border px-3 py-2 text-sm" placeholder="Tên ma trận đề" />
            ...
            <button class="rounded-md bg-slate-950 px-3 py-2 text-sm font-semibold text-white" @click="createBlueprint">Lưu ma trận đề</button>
          </div>
        </div>
        <div class="border bg-white p-4">
          <h2 class="text-sm font-semibold">Xem trước đề ngẫu nhiên</h2>
          <p class="mt-2 text-sm text-slate-600">{{ selected?.name || 'Chưa chọn ma trận đề' }}</p>
          <button class="mt-4 w-full rounded-md bg-slate-950 px-3 py-2 text-sm text-white" @click="generatePreview">Sinh bản xem trước</button>
          <div v-if="preview" class="mt-3 max-h-72 overflow-auto rounded-md border border-slate-200">
            <div class="border-b bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600">Bản xem trước {{ previewQuestions.length }} câu</div>
            <div class="divide-y divide-slate-100">
              <div v-for="(question, index) in previewQuestions" :key="question.id || index" class="px-3 py-2 text-sm">
                <div class="font-semibold text-slate-900">{{ index + 1 }}. {{ question.title || question.prompt || question.question_text || `Câu hỏi ${question.id || index + 1}` }}</div>
                <div class="mt-1 text-xs text-slate-500">{{ question.question_type || question.type || 'câu hỏi' }} · {{ difficultyLabel[question.difficulty] || question.difficulty || '-' }} · {{ question.default_score || question.score || 0 }} điểm</div>
              </div>
              <div v-if="!previewQuestions.length" class="px-3 py-6 text-sm text-slate-500">Ma trận đề đã chạy nhưng chưa trả câu hỏi xem trước.</div>
            </div>
          </div>
        </div>
      </aside>
