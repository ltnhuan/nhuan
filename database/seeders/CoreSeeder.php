...
class CoreSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $permissions = ['core.tenant.view','core.tenant.manage','core.user.view','core.user.create','core.user.update','core.user.lock','core.role.manage','course.view','course.create','course.update','course.delete','course.review','course.approve','course.publish','lesson.manage','quiz.manage','assignment.manage','assignment.view','assignment.create','assignment.update','assignment.submit','assignment.grade','assignment.approve','rubric.manage','grade.view','grade.approve','sis.sync.manage','report.view','ai.use','repository.view','repository.upload','repository.manage','repository.approve','learning_path.view','learning_path.manage','progress.view_own','progress.view_class','progress.recalculate','completion.approve','completion.override','video.view','video.upload','video.manage','video.attach','video.analytics.view','video.process','question_bank.view','question_bank.manage','question_bank.approve','question.view','question.create','question.update','question.approve','question.publish','question.import','blueprint.manage','exam.view','exam.create','exam.update','exam.publish','exam.assign','exam.attempt','exam.grade','exam.result.view','exam.result.publish','exam.proctor.view','gradebook.view','gradebook.manage','gradebook.calculate','grade.override','grade.approve','grade.lock','grade.sync_sis','live_session.view','live_session.manage','attendance.view','attendance.manage','attendance.checkin','attendance.lock','eligibility.view','eligibility.recalculate','integration.view','integration.manage','integration.push','api_ops.view','api_ops.system.manage','api_ops.endpoint.manage','api_ops.request.view','api_ops.payload.view','api_ops.secret.manage','api_ops.webhook.manage','api_ops.mapping.manage','api_ops.sync.run','api_ops.sync.retry','api_ops.health.view','api_ops.console.use','api_ops.contract.manage','enrollment.view','enrollment.manage','enrollment.import','enrollment.analytics','enrollment.assign_teacher','enrollment.self','analytics.dashboard','analytics.risk','analytics.alerts','analytics.warehouse','career.view','career.manage_own','portfolio.manage_own','skill.manage','competency.manage','employer.view','credential.view','credential.manage','credential.issue','credential.revoke','credential.wallet','obe.view','obe.manage','obe.analytics','accreditation.report','community.view','community.manage','community.post','community.wiki','community.blog','community.group','community.reputation','community.moderate','community.analytics','survey.view','survey.manage','survey.respond','survey.analytics','survey.export','improvement.view','improvement.manage','accreditation.evidence.view','accreditation.evidence.manage'];
        ...
        $roles = ['super_admin'=>'Quản trị hệ thống','tenant_admin'=>'Quản trị tenant','integration_admin'=>'Quản trị tích hợp API','api_ops_viewer'=>'Người xem API Operations','campus_admin'=>'Quản trị cơ sở','academic_admin'=>'Quản trị đào tạo','faculty_manager'=>'Quản lý khoa','training_officer'=>'Cán bộ đào tạo','teacher'=>'Giảng viên','student'=>'Sinh viên','parent'=>'Phụ huynh','external_learner'=>'Học viên ngoài','sis_integration_bot'=>'Bot tích hợp SIS'];
        foreach ($roles as $name => $display) {
            ...
            $allowed = match ($name) {
                'student' => ['course.view','progress.view_own','video.view','exam.view','exam.attempt','exam.result.view','assignment.view','assignment.submit','gradebook.view','attendance.checkin','eligibility.view','enrollment.view','enrollment.self','analytics.dashboard','career.view','career.manage_own','portfolio.manage_own','skill.manage','competency.manage','credential.view','credential.wallet','ai.use','community.view','community.post','community.wiki','community.blog','community.group','community.reputation','survey.view','survey.respond'],
                'api_ops_viewer' => ['api_ops.view','api_ops.request.view','api_ops.health.view'],
                'integration_admin' => ['integration.view','integration.manage','integration.push','api_ops.view','api_ops.system.manage','api_ops.endpoint.manage','api_ops.request.view','api_ops.payload.view','api_ops.webhook.manage','api_ops.mapping.manage','api_ops.sync.run','api_ops.sync.retry','api_ops.health.view','api_ops.console.use','api_ops.contract.manage'],
                default => $permissions,
            };
        ...
        foreach ([['admin.lms@vabis.edu.vn','ADMIN','Quản trị LMS','admin'],['daotao.lms@vabis.edu.vn','DAOTAO','Cán bộ đào tạo','staff'],['khoa.lms@vabis.edu.vn','KHOA','Quản lý khoa','staff'],['gv.lms@vabis.edu.vn','GVDEMO','Giảng viên Demo','teacher'],['sv.lms@vabis.edu.vn','SVDEMO','Sinh viên Demo','student']] as [$email, $code, $name, $type]) {
            LmsUser::query()->updateOrCreate(['tenant_id' => $tenant->id, 'email' => $email], ['code' => $code, 'full_name' => $name, 'user_type' => $type, 'status' => 'active', 'metadata' => ['demo' => true, 'demo_password' => 'admin123456']]);
        }
        for ($i = 1; $i <= 90; $i++) {
